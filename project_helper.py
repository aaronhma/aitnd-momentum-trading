...
# Start the Jupyter Notebook mode
offline_py.init_notebook_mode(connected = True)

def _generate_stock_trace(prices):
    """
    Private implementation of generating the stock scatter plot.
    """
    return go.Scatter(
        name = 'Index',
        x = prices.index,
...
def _generate_traces(name_df_color_data):
    """
    Private implementation of generating scatter plot.
    """
    traces = []
...
def print_dataframe(df, n_rows = 10, n_columns = 3):
    """
    A utility function to show the Pandas DataFrame.
    """
    missing_val_str = '...'
    config = helper.generate_config()
...
def plot_shifted_returns(df_shited, df, title):
    """
    A utility function to plot the pruces after shifted returns.
    """
    config = helper.generate_config()
    layout = go.Layout(title = title)
...
def print_top(df, name, top_n = 10):
    """
    A utility function to print the top 10 stocks.
    """
